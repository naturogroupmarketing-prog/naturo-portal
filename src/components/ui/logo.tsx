...
export function Logo({ size = 32, className = "", iconOnly = false }: LogoProps) {
  ...
  return (
    <span className={`inline-flex items-center select-none ${className}`} style={{ gap: `${size * 0.08}px` }}>
      {/* Plug/connector icon matching brand screenshot */}
      <svg
        width={iconSize}
        height={iconSize}
        viewBox="0 0 48 48"
