...
type State = "idle" | "ready" | "installed" | "guide";
...
export function InstallAppSection() {
  ...
  // iOS Safari — show guide inline or button to reveal guide
      ...
      <div>
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs font-semibold text-action-600 hover:text-action-700 px-3 py-1.5 bg-action-50 rounded-lg"
          >
            {showIOSGuide ? "Close" : "How to"}
          </button>
        </div>
        {showIOSGuide && (
          <div className="mt-3 bg-shark-50 dark:bg-shark-800 rounded-xl p-4 space-y-3">
            ...
            <ol className="space-y-2.5">
              {[
                <>Tap the <strong>Share</strong> button <svg className="inline mb-0.5" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg> at the bottom of Safari</>,
  ...
  // Android / Desktop — native prompt ready
  ...
  // Android / Desktop — no prompt yet (Chrome hasn't offered it)
  if (platform === "android" || platform === "desktop") {
    return (
      <div className="flex items-center gap-3 py-1">
        ...
        <div>
          <p className="text-sm font-medium text-shark-800 dark:text-shark-200">Install App</p>
          <p className="text-xs text-shark-400 mt-0.5 leading-relaxed">
            Look for the <strong>install icon (⊕)</strong> in Chrome's address bar, or tap <strong>⋮ → Install app</strong>
          </p>
        </div>
      </div>
    );
