...
export function LocationMap({ locations, className = "" }: LocationMapProps) {
  ...
  if (locations.length === 0) {
    return (
      <div className={`flex items-center justify-center bg-shark-50 rounded-[20px] text-shark-400 text-sm ${className}`} style={{ minHeight: 300 }}>
  ...
  return (
    <>
      ...
      `}</style>
      <div ref={mapRef} className={`rounded-[20px] overflow-hidden ${className}`} style={{ minHeight: 300 }} />
    </>
  );
}
