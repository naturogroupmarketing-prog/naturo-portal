...
export function Sidebar({ role, onClose, pendingPOCount = 0, pendingReturnsCount = 0 }: SidebarProps) {
  ...
  return (
    <nav aria-label="Main navigation" className="flex flex-col h-full">
                  ...
                  {visibleItems.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          "flex items-center gap-3 px-3 py-2.5 text-sm rounded-lg min-h-[44px] transition-all duration-150",
                          active
                            ? "bg-white/10 text-white font-medium"
                            : "text-[#a0b3c8] hover:bg-white/[0.06] hover:text-white"
                        )}
                      >
                        <Icon
                          name={item.icon}
                          size={17}
                          className={active ? "text-action-300" : "text-[#697386]"}
                        />
      ...
      <div className="hidden lg:flex items-center gap-2 px-3 py-3 border-t border-white/[0.08]">
        <button
          onClick={() => document.dispatchEvent(new KeyboardEvent("keydown", { key: "k", metaKey: true }))}
          className="flex items-center gap-2 w-full px-2.5 py-1.5 rounded-lg text-[#697386] hover:text-[#a0b3c8] hover:bg-white/[0.06] transition-colors text-xs"
        >
          <Icon name="search" size={13} />
          <span className="flex-1 text-left">Quick search</span>
          <kbd className="text-[10px] bg-white/10 border border-white/10 px-1 py-0.5 rounded font-mono text-[#697386]">⌘K</kbd>
        </button>
      </div>
...
function CollapsibleSection({ heading, children, className }: { heading?: string; children: React.ReactNode; className?: string }) {
  ...
  return (
    <div className={className}>
      ...
      >
        <span className="text-[10px] font-semibold uppercase tracking-widest text-[#4f566b] group-hover:text-[#697386] transition-colors">
          {heading}
        </span>
        <svg
          ...
          className={cn("text-[#4f566b] transition-transform duration-200", collapsed ? "-rotate-90" : "")}
        >
          <path d="M6 9l6 6 6-6" />
        </svg>
      </button>
