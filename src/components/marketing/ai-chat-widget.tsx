    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-[360px] max-w-[calc(100vw-2rem)] animate-[fadeInUp_0.25s_ease-out]">
          <div className="bg-white rounded-[20px] border border-shark-200 shadow-2xl shadow-shark-300/30 overflow-hidden flex flex-col" style={{ height: "520px" }}>
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-shark-50/30 dark:bg-shark-800/30">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div
                    className={`max-w-[85%] px-3.5 py-2.5 rounded-[20px] text-[13px] leading-relaxed ${
                      msg.role === "user"
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-white border border-shark-100 dark:border-shark-800 rounded-[20px] rounded-bl-md px-4 py-3 shadow-sm">
                ...
                <div className="space-y-1.5 pt-1">
                  ...
                  {SUGGESTED_QUESTIONS.map((q) => (
                    <button
                      ...
                      className="block w-full text-left text-[12px] text-action-600 bg-white hover:bg-action-50 border border-shark-100 dark:border-shark-800 hover:border-action-200 rounded-[20px] px-3 py-2 transition-colors"
                    >
                      {q}
                    </button>
                  ))}
            ...
            <div className="border-t border-shark-100 dark:border-shark-800 px-3 py-3 bg-white shrink-0">
              ...
              >
                <input
                  ...
                  className="flex-1 text-sm text-shark-800 placeholder:text-shark-400 bg-shark-50 border border-shark-100 dark:border-shark-800 rounded-[20px] px-3.5 py-2.5 focus:outline-none focus:border-action-300 focus:ring-2 focus:ring-action-100 transition-all"
                />
                <button
                  type="submit"
                  disabled={!input.trim()}
                  className="w-9 h-9 rounded-[20px] bg-action-500 hover:bg-action-600 disabled:bg-shark-200 dark:bg-shark-700 disabled:cursor-not-allowed flex items-center justify-center transition-colors shrink-0"
                >
