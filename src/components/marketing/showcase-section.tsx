...
const steps = [
    ...
    visual: (
      <div className="space-y-3 p-6">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-[28px] bg-blue-50 flex items-center justify-center">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={NAVY} strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
          </div>
          <div>
            <p className="text-sm font-bold text-gray-900">Your Locations</p>
            <p className="text-[11px] text-gray-400">3 states · 8 branches</p>
          </div>
        </div>
        {[
          { state: "New South Wales", count: 4, color: "bg-blue-500" },
          { state: "Victoria", count: 2, color: "bg-indigo-500" },
          { state: "Queensland", count: 2, color: "bg-amber-500" },
        ].map((s) => (
          <div key={s.state} className="flex items-center gap-3 p-3 rounded-[28px] bg-white border border-gray-100">
            <div className={`w-2.5 h-2.5 rounded-full ${s.color}`} />
            <span className="text-sm font-medium text-gray-800 flex-1">{s.state}</span>
            <span className="text-xs text-gray-400">{s.count} locations</span>
    ...
    visual: (
      <div className="space-y-2.5 p-6">
        <div className="flex items-center justify-between mb-3">
          <p className="text-sm font-bold text-gray-900">Assets</p>
          <span className="text-[10px] font-medium text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full">255 total</span>
        </div>
        {[
          ...
          <div key={a.name} className="flex items-center gap-3 p-2.5 rounded-[28px] bg-white border border-gray-100">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-medium text-gray-800 truncate">{a.name}</p>
              <p className="text-[10px] text-gray-400">{a.assignee}</p>
            </div>
            <span className="flex items-center gap-1.5">
              <span className={`w-1.5 h-1.5 rounded-full ${a.statusColor}`} />
              <span className="text-[10px] font-medium text-gray-500">{a.status}</span>
            </span>
          </div>
        ))}
      </div>
    ...
    visual: (
      <div className="space-y-2.5 p-6">
        ...
        {[
          ...
          { type: "New Request", msg: "Mia L. requested 50x bin liners", time: "2 hours ago", color: "bg-indigo-500", bgColor: "bg-indigo-50" },
        ].map((n) => (
          <div key={n.msg} className="flex items-start gap-3 p-3 rounded-[28px] bg-white border border-gray-100">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[11px] font-semibold text-gray-800">{n.type}</p>
              <p className="text-[10px] text-gray-400 leading-snug">{n.msg}</p>
            </div>
            <span className="text-[9px] text-gray-300 whitespace-nowrap shrink-0">{n.time}</span>
          </div>
        ))}
      </div>
    ),
  },
  {
    ...
    description:
      "One dashboard for every branch. See operations health, stock levels, and team activity at a glance.",
    icon: (
      <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round">
    ...
    visual: (
      <div className="p-6 space-y-3">
        <p className="text-sm font-bold text-gray-900 mb-3">Operations Overview</p>
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={s.label} className="bg-white rounded-[28px] border border-gray-100 p-2.5 text-center">
        ...
        <div className="bg-white rounded-[28px] border border-gray-100 p-3 flex items-center gap-3">
          <div className="w-12 h-12 rounded-[28px] bg-green-500 flex items-center justify-center text-white text-lg font-bold">94</div>
          <div>
            <p className="text-[12px] font-semibold text-gray-800">Operations Health</p>
            <p className="text-[10px] text-gray-400">All branches performing well</p>
          </div>
        </div>
        <div className="bg-white rounded-[28px] border border-gray-100 p-3">
          <p className="text-[11px] font-semibold text-gray-700 mb-2">Asset Utilisation by Branch</p>
          <div className="space-y-1.5">
            {[
              ...
              <div key={b.branch} className="flex items-center gap-2">
                <span className="text-[9px] text-gray-500 w-10">{b.branch}</span>
                <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${b.pct}%`, background: NAVY }} />
                </div>
                <span className="text-[9px] text-gray-500 w-7 text-right">{b.pct}%</span>
              </div>
            ))}
          </div>
        </div>
...
export function ShowcaseSection() {
  ...
  return (
    <section id="how-it-works" className="py-14 sm:py-20 lg:py-28 bg-gray-50 overflow-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="relative order-2 lg:order-1">
            <div className="relative">
              {steps.map((step, i) => (
                ...
                >
                  <div className="bg-white rounded-[28px] border border-gray-200 shadow-xl shadow-gray-200/30 overflow-hidden">
                    <div className="flex items-center gap-2 px-4 py-2 border-b border-gray-100 bg-gray-50/50">
                      ...
                      <div className="flex-1 flex justify-center">
                        <div className="bg-white rounded-md border border-gray-200 px-3 py-0.5 text-[10px] text-gray-400 flex items-center gap-1.5">
                    ...
                    <div className="bg-gray-50 h-[280px] sm:h-[340px] lg:h-[380px] overflow-y-auto">
                      {step.visual}
                    </div>
                  </div>
                </div>
          ...
          <div className="space-y-0 order-1 lg:order-2">
            {steps.map((step, i) => {
              ...
              return (
                <button
                  ...
                  className="relative w-full text-left transition-colors duration-500 ease-out overflow-hidden hover:bg-gray-100"
                ...
                >
                    ...
                    <div className="absolute bottom-0 left-0 right-0 h-[3px]">
                      <div
                        ...
                        style={{
                          ...
                          transition: animating ? `width ${STEP_DURATION}ms linear` : "none",
                        }}
                      />
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-5 px-6 py-5">
                    <div className={`shrink-0 transition-colors duration-500 ${isActive ? "text-white" : "text-blue-400"}`}>
                      {step.icon}
                    </div>
                    <h3 className={`text-lg font-semibold transition-colors duration-500 ${isActive ? "text-white" : "text-gray-900"}`}>
                      {step.title}
                    </h3>
                  </div>
            ...
            <div className="pt-6">
              <Link
                href="/login"
                className="inline-flex items-center justify-center text-sm font-bold px-8 py-3.5 rounded-full transition-all hover:-translate-y-px hover:shadow-lg active:scale-[0.97] text-white"
