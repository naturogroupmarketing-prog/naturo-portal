...
export async function batchUpdatePOStatus(poIds: string[], newStatus: string) {
  ...
  if (newStatus === "APPROVED" || newStatus === "REJECTED" || newStatus === "ORDERED") {
    const canApprove = session.user.role === "SUPER_ADMIN" || await hasPermission(session.user.id, session.user.role, "purchaseOrderApprove");
    if (!canApprove) throw new Error("You don't have permission to approve/reject orders");
  }
  if (newStatus === "RECEIVED") {
    if (!(await hasPermission(session.user.id, session.user.role, "purchaseOrderManage"))) {
      throw new Error("You don't have permission to receive orders");
    }
  }

  // Fetch and validate all POs
  const pos = await db.purchaseOrder.findMany({
    where: { id: { in: poIds }, organizationId },
    include: { consumable: true },
  });

  if (pos.length === 0) throw new Error("No purchase orders found");
