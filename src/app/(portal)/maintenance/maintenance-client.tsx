...
function getDueStatus(dueDate: string) {
  ...
  if (daysUntil === 0) return { label: "Due Today", color: "bg-amber-100 text-amber-700", urgent: true };
  if (daysUntil <= 7) return { label: `Due in ${daysUntil}d`, color: "bg-amber-50 text-amber-600", urgent: false };
  return { label: `Due in ${daysUntil}d`, color: "bg-emerald-50 text-emerald-600", urgent: false };
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-shark-900">Maintenance</h1>
      ...
      <div className="flex gap-2">
        {(["all", "overdue", "upcoming"] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${filter === f ? "bg-action-500 text-white" : "bg-shark-100 text-shark-600 hover:bg-shark-200"}`}
          >
