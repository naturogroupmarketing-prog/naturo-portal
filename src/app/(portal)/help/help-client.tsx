...
const FAQ_SECTIONS: FAQSection[] = [
  ...
  },
  {
    ...
    icon: "package",
    faqs: [
      { q: "How do I view my inventory?", a: "Click 'Inventory' in the sidebar. You'll see all your locations grouped by state. Click any location to see its assets, supplies, and staff all on one page." },
      ...
      { q: "How do I set up a new location quickly?", a: "When you open an empty location in Inventory, you'll see a 'Set Up This Location' card. Click 'Apply Standard Items' to copy all asset and supply types from your existing locations with one click." },
      ...
      { q: "How do I archive a location?", a: "In Inventory, click the archive icon (↓) next to the edit icon on any region card. A confirmation shows how many assets, supplies, and staff will be affected. Everything is preserved — you can restore the location anytime from the 'Archived Locations' section at the bottom of the Inventory page.", roles: ["SUPER_ADMIN"] },
      { q: "How do I restore an archived location?", a: "At the bottom of the Inventory page, click 'X archived locations' to expand the list. Click 'Restore' on the location you want to bring back. All assets, supplies, and staff reappear immediately.", roles: ["SUPER_ADMIN"] },
  ...
  },
  {
    ...
    icon: "droplet",
    faqs: [
      { q: "How do I check stock levels?", a: "Go to Inventory → click a location → Supplies tab. You'll see all items with their current stock, minimum threshold, and reorder level. Items below threshold are flagged." },
      { q: "How do I add or deduct stock?", a: "Click the 'Stock' button next to any supply item. Use the toggle to switch between Add and Deduct. Enter the quantity and (for deductions) a reason. Stock updates immediately.", roles: ["SUPER_ADMIN", "BRANCH_MANAGER"] },
      { q: "How do I request supplies as a staff member?", a: "Go to 'Request Supplies' in the sidebar. Browse or search for the item you need, enter the quantity, and submit. Your manager will be notified and can approve or reject the request." },
      { q: "What happens when stock is low?", a: "When a supply drops below its minimum threshold, a purchase order is automatically created and managers are notified by email and in-app notification. The item appears in the Low Stock alerts on the dashboard." },
      { q: "How do I approve supply requests?", a: "Go to Supplies → Requests tab. You'll see all pending requests. Click 'Approve' or 'Reject' on each one. You can also batch approve multiple requests at once.", roles: ["SUPER_ADMIN", "BRANCH_MANAGER"] },
      { q: "How do I edit stock directly?", a: "When editing a supply item, there's a 'Current Stock' field (visible to Super Admin and managers with stock adjust permission). Change the number and save to set the exact quantity.", roles: ["SUPER_ADMIN", "BRANCH_MANAGER"] },
    ],
  },
  {
    ...
    icon: "truck",
    faqs: [
      { q: "How are purchase orders created?", a: "Purchase orders are created automatically when supply stock drops below the threshold. They can also be created manually from the Purchase Orders page or via the AI Assistant." },
      ...
      { q: "How do I mark an order as received?", a: "When an ORDERED purchase order arrives at your location, click 'Received'. This automatically adds the quantity to your supply stock." },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "How do I view what equipment a staff member has?", a: "Go to Staff → click 'View' on any staff member. This shows all their assigned assets, supplies, supply usage history, damage reports, and performance metrics." },
  ...
  },
  {
    ...
    icon: "box",
    faqs: [
      { q: "What is a starter kit?", a: "A starter kit is a template of assets and supplies that new staff members need. When you apply a kit, it automatically assigns all the items from your location's inventory to that person." },
      { q: "How do I create a starter kit?", a: "Go to Starter Kits → click '+ New Kit'. Name it, then add items using the '+ Add Item' button. You can add asset categories (like 'Vacuum', 'Mop Bucket') and specific supplies with quantities.", roles: ["SUPER_ADMIN", "BRANCH_MANAGER"] },
      ...
      { q: "Do I need supplies in every location before applying a kit?", a: "Yes. The starter kit finds supplies by name in the staff member's specific region. If a supply exists in Port Macquarie but not in Geelong, it will be skipped for Geelong staff. Use 'Apply Standard Items' on empty locations in Inventory to quickly copy all supply types from existing locations." },
  ...
  },
  {
    ...
    icon: "clipboard",
    faqs: [
      { q: "What reports are available?", a: "Asset Register, Supplies by Location, Staff Assignments, Staff Supply Usage, Stock Movement, Request History, Damage & Loss, Audit Trail, and Maintenance Schedule. All exportable as CSV." },
      ...
      { q: "How do I download a full backup?", a: "Go to Admin → Backup. Click 'Download All' to get 4 CSV files: assets, supplies, staff, and regions. You can also download each individually. These files can be re-uploaded via Import Data to restore everything.", roles: ["SUPER_ADMIN"] },
      { q: "How do I restore from a backup?", a: "Go to Admin → Import Data. Upload any of the backup CSV files. The column headers are automatically matched. Assets and supplies are created in their original regions with unique codes.", roles: ["SUPER_ADMIN"] },
  ...
  },
  {
    ...
    icon: "lock",
    faqs: [
      { q: "What are the user roles?", a: "Super Admin — full access to everything. Branch Manager — manages their assigned location (assets, supplies, staff, returns). Staff — views their assigned items, requests supplies, reports damage, completes inspections." },
...
export function HelpClient({ role }: { role: string }) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        <h1 className="text-[28px] sm:text-[32px] font-bold text-shark-900 dark:text-shark-100 leading-tight tracking-tight">Help & Support</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card className="hover:shadow-md transition-shadow">
          <div className="px-5 py-4 flex items-center gap-4">
            <div className="w-10 h-10 rounded-[20px] bg-action-500 flex items-center justify-center shrink-0">
              ...
              <Card key={section.title} className="overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-[20px] bg-action-500 flex items-center justify-center">
                  ...
                  <div className="border-t border-shark-100 dark:border-shark-700 px-5 py-2">
                    {section.faqs.map((faq, i) => (
                      <details key={i} className="group" open={!!search}>
                        <summary className="flex items-center justify-between cursor-pointer text-sm font-medium text-shark-700 dark:text-shark-300 hover:text-shark-900 dark:text-shark-100 py-3 border-b border-shark-50 dark:border-shark-800 last:border-0">
                          {faq.q}
